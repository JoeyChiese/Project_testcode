#http://mp.weixin.qq.com/s?src=3&timestamp=1491902320&ver=1&signature=Q7ozP8fGOZLYT5E2pYM3mBM6ymc8sY2N9w9-z2tQr970jXRwA1HINDn8oXXUErllDKCHUl1lxO4YxGYxx4uFeigbeMjUiREKuh*6IahrxG-ULAdTLO6J4WSIjedEhQCp2Y-kTMBzZEgPpVmQ4c-oyA==
...
set.seed(123)
dataset = na.omit(dataset)#去掉为空的行
n = dim(dataset)[1]
...
dataset2 = colwise(into_factor)(dataset2)#colwise(fun)(data) column-wise function
...
accurancy1 = mean(logit.predict == test$V16)#准确率
accurancy1

#GBM
#使用GBM方法来进行预测,这里用的是caret,repeat-cv来选择最优树
library(caret)
ctrl = trainControl(method = "repeatedcv",number = 5, repeats = 5)#control parameters for train
set.seed(300)
...
#首先测试一个knn模型,不做CV,不做标准化,
#不做数据类型转换得到的结果,这里,不转换数据类型会把因子类型的变量舍弃,仅保留数值变量
library(caret)
...
knnTune <- train(x = dataset2[1:37], y = dataset2[,38], method = "knn",
...
#xgboost
#Install:devtools::install_github('dmlc/xgboost',subdir='R-package')
require(xgboost)
require(methods)
require(plyr)
set.seed(123)
...
param <- list("objective" = "binary:logistic", "bst:eta" = 1, "bst:max_depth" = 2,eval_metric = "logloss",
              "silent" = 1,  "nthread" = 16 , "min_child_weight" =1.45)
nround =275
